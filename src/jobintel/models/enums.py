...
`StrEnum` rather than the older `(str, Enum)` mixin: both compare equal to
their string value, but StrEnum also formats as its value, so an accidental
f-string interpolation yields "REMOTE_WORLDWIDE" instead of
"RemoteClassification.REMOTE_WORLDWIDE" leaking into a digest or an alert.
Everything persisted still goes through `.value` explicitly.
"""
